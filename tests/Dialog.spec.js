...
describe('Dialog', () => {
  ...
  it('does not render dialog content when closed', () => {
    ...
    // Dialog appends to body in mounted(); check there is no visible wrap.
    expect(document.querySelector('.ui-dialog-wrap')).toBeNull()
    w.unmount && w.unmount()
  })

  it('renders dialog content when open', async () => {
    const w = mount(Dialog, { props: { open: true, title: 'Hello' } })
    await w.vm.$nextTick()
  ...
  it('emits change(false) and cancel when close button is clicked', async () => {
    const w = mount(Dialog, { props: { open: true, closable: true } })
    await w.vm.$nextTick()
  ...
  it('renders title and sub-text props', async () => {
    const w = mount(Dialog, { props: { open: true, title: 'T', subText: 'S' } })
    await w.vm.$nextTick()
  ...
  it('respects showHeader=false', async () => {
    const w = mount(Dialog, { props: { open: true, showHeader: false, title: 'T' } })
    await w.vm.$nextTick()
  ...
  it('hides footer when footer=false', async () => {
    const w = mount(Dialog, { props: { open: true, footer: false } })
    await w.vm.$nextTick()
