...
describe('Button', () => {
  it('renders label prop', () => {
    const w = mount(Button, { props: { label: 'Click me' } })
  ...
  it('does not emit click when disabled', async () => {
    const w = mount(Button, { props: { label: 'X', disabled: true } })
    await w.trigger('click')
    expect(w.emitted('click')).toBeFalsy()
  })

  it('does not emit click when loading and shows spinner', async () => {
    const w = mount(Button, { props: { label: 'X', loading: true } })
  ...
  it('sets the native button type attribute', () => {
    const w = mount(Button, { props: { label: 'X', htmlType: 'submit' } })
