...
### Added
- `WkCheckbox` and `WkCheckboxGroup` accept a new `iconType` prop (`'default'` or `'minus'`) to render an indeterminate minus icon instead of the default check icon when checked.
...
### Fixed
- `wkMessage` now falls back to the global `placement` and `offset` config when a per-call value isn't provided, instead of leaving the container unpositioned.

## [1.3.8] - 2026-09-16

### Added

- `WkiLineThrough` icon is now exported as an alias for the existing `WkiStrikethrough` icon.

## [1.3.7] - 2026-09-16

### Fixed

- `WkBreadcrumb`'s current item no longer applies a bold font-weight override, matching the weight of the other breadcrumb items.
- `WkTag`'s `outline` type now uses the `--primary-bg` and `--primary-hover` design tokens instead of hardcoded translucent colors for its background and hover/active states.
- `WkTag`'s `lg` size now uses the `--radius` token instead of `--rounded-xl` for its border radius.
...
- `WkTabs` accepts a new `mini` size.
- New `WkiLettersSpacing` icon.

### Changed

- `WkTabs` reworks its size scale (`mini`, `xs`, `sm`, `md`, `lg`) with updated padding, gap, border-radius, and typography per size, and its icons no longer force a fixed width/height.
- `WkSelect`'s search placeholder font size now inherits from its container instead of a fixed value.

### Fixed

- `WkTableV2` no longer removes the bottom border on the last body row, keeping borders consistent when bordered.
